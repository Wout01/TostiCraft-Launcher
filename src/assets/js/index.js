...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "For questions you can visit our forum." },
            { "message": "Don't forget to join our discord server." },
            { "message": "Found a bug? Let us know through our forum." }
        ]
    ...
    async checkUpdate() {
        ...
        ipcRenderer.invoke('update-app').then(err => {
            if (err.error) {
                ...
                this.shutdown(`error while searching for update :<br>${error}`);
            }
        })
